// augmented-citizen-mcp-server/src/security/mod.rs
...
pub fn allowed_for(verdict: ConsentVerdict, method: &str) -> AccessLevel {
    ...
    match risk {
        ToolRisk::High => match level {
            ...
            AccessLevel::Restricted => AccessLevel::Restricted, // e.g., “dry-run only”
        ...
        ToolRisk::Low => match level {
            ...
            AccessLevel::Deny => AccessLevel::Restricted, // still permit read-only, audited.
        },
    }
}
